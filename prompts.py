...
- Include all ATTRIBUTES as keys. Empty cells are allowed and must be empty strings.
...
- For dict keys: use ASCII tag names; if invalid or starting with 'xml', use <field>.
...
XML RULES:
- Root element is <root>.
- Map keys -> child elements (ASCII; invalid names or starting with 'xml' -> <field>).
...
XML RULES:
- Root element is <root>.
- Each row -> <item> with child elements for each column.
- Use ASCII tag names; otherwise use <field>.
...
- Child tag names must be ASCII; invalid or 'xml*' -> <field>.
...
- Keys must be TOML-safe; quote unsafe keys. Null -> empty string.
